<?php  
  $edit=mysql_query("SELECT * FROM tempat WHERE id_tempat='$_GET[id]'")or die(mysql_error());
...
<main role='main'>
      ...
      <article>
        <header class='section background-primary text-center'>
            <h1 class='text-white margin-bottom-0 text-size-50 text-thin text-line-height-1'>UBAH KOSAN</h1>
        </header>
        <div class='section background-white'> 
          <div class='line'>
            <div class='margin'>
              <h2 class='text-uppercase text-strong margin-bottom-30'>UBAH DATA KOSAN</h2>
              <!-- Company Information -->
                ...
                <div class='margin-left-80 margin-bottom'>
                  ...
                  <div class='line'>
                   ...
                   <div class='s-11'> 
                    ...
                    <input name='nama_kosan' value='<?php echo "$r[nama_tempat]";?>' class='subject border-radius' placeholder='Nama kosan' title='Nama kosan' type='text' required/>
                  </div>
                  ...
                  <div class='s-11'>
                    ...
                    <textarea name='alamat_lengkap' class='required message border-radius' placeholder='Alamat lengkap' rows='3' required><?php echo "$r[alamat]";?></textarea>
                  </div>
                  <div class='s-11'>
                  <label>Kategori</label>
                    ...
                    <select name='id_kategori' class='form-control' required><?php
                      ...
                      <div class='s-11'>
                        ...
                        <input name='luas_kamar' value='<?php echo "$r[luas_kamar]";?>' required class='name border-radius' placeholder='Luas kamar (tanpa KM) 3 x 3 M' title='Luas kamar (tanpa KM)' type='text' />
                      </div>
                      <div class='s-11'>
                        <label>Jumlah Kamar</label>
                        <input name='jml_kamar' value='<?php echo "$r[jml_kamar]";?>' required class='required email border-radius' placeholder='Jumlah kamar' title='Jumlah kamar' type='text' />
                      </div>
                      <div class='s-11'>
                        <label>Kamar Kosong</label>
                        <input name='jml_kosong' value='<?php echo "$r[jml_kosong]";?>' required class='name border-radius' placeholder='Jumlah Kamar Kosong' title='Jumlah Kamar Kosong' type='text' />
                      </div>
                      <div class='s-11'>
                        <label>Tarif Kost</label>
                         <select name='tarif' required>
                         <option value='0'>- Pilih Tarif -</option>
                         <option value='Bulanan' <?php if($r[tarif]=="Bulanan"){echo "selected";}?>>Bulanan</option>
                         <option value='Harian' <?php if($r[tarif]=="Harian"){echo "selected";}?>>Harian</option>
                         <option value='Mingguan' <?php if($r[tarif]=="Mingguan"){echo "selected";}?>>Mingguan</option>
                         </select>
                      </div>
                      <div class='s-11'>
                          <img src="foto_fasilitas/<?php echo "$r[gambar]";?>" width="100">
                      </div>
              ...
              <div class='s-12 m-12 l-6'>
                  ...
                  <div class='s-12 m-12 l-12'>
                    <label>Harga Tarif</label>
                        <input name='harga_tarif' required value='<?php echo "$r[harga_tarif]";?>' class='name border-radius' placeholder='Sesuai tarif per' title='Sesuai tarif per' type='text' />
                  </div>
                  

                  <div class='s-12'>
                    <label>Lokasi (Longitude, Latitude)</label>
                    <textarea name='petalokasi' required  class='required message border-radius' placeholder='Kordinat Peta Lokasi' rows='2'><?php echo "$r[kordinat]";?></textarea>
                  </div>
                  ...
                  <div class='s-12' >
                    <label>Fasilitas</label>
                      <select name='fasilitasdrp' id='dropdown' required>
                              <option value='' <?php if($r[fasilitas]==""){echo "selected";}?>>- Pilih Fasilitas -</option>
                              <option value='Isi' <?php if($r[fasilitas]=="Isi"){echo "selected";}?>>Isi</option>
                              <option value='Kosong' <?php if($r[fasilitas]=="Kosong"){echo "selected";}?>>Kosong</option>
                     </select>
                  </div>            
                   
                  <div class='s-12'>
                    <label>Keterangan Fasilitas</label>
                    <textarea name='fasilitas' required class='required message border-radius' id='ketfas' placeholder='Fasilitas' rows='3'><?php echo "$r[detail_fasilitas]";?></textarea>
                  </div>
                 ...
                 <div class='s-12'>
                    <div class='s-12'>
                    <textarea name='frame' required class='required message border-radius' placeholder='URL Frame ' rows='3'><?php echo "$r[frame]";?></textarea>
                    </div>
                 </div>        
                  <div class='s-12'>
...
$nopel=$pel.substr($nourut,-3);
  ...
  if(isset($_POST[tambahkan])){
    include"config/upload_foto.php";
      ...
      $qry = mysql_query("UPDATE tempat SET 
                          id_kategori       ='$kategori',
                          nama_tempat       ='$nama_kost',
                          keterangan        ='$lainnya',
                          alamat            ='$alamat_lengkap',
                          luas_kamar        ='$luas_kamar',
                          jml_kamar         ='$Jumlah_kamar',
                          jml_kosong        ='$kamar_kosong',
                          tarif             ='$tarif',
                          harga_tarif       ='$harga_tarif',
                          fasilitas         ='$fasilitasdrp',
                          detail_fasilitas  ='$fasilitas',
                          kordinat          ='$petalokasi',
                          gambar            ='$nama_file_unik',
                          frame             ='$frame',
                          id_kota           ='$kota'
                          WHERE id_tempat   = '$idt'")or die(mysql_error());
      if($qry){
